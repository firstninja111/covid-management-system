...
class DashboardController extends Controller
{
     ...
     * @return void
     */
    public function __construct(Charts $chart, User $user, Subscriptions $subscriptions, StripeCustomer $stripeCustomer, Appointment $appointment)
    {
    ...
    {
        $sum = 0;
        $appointment_fetch = $this->appointment->query()->where('payment_status', 'Paid')->get();
        foreach($appointment_fetch as $appointment){
            $sum += $appointment->paid;
        }
        return $sum;
